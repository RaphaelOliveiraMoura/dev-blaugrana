...
const DIR = path.join(AQUI, '_icone-conceitos');
...
const candidatas = [['meio-tom (a régua)',
  await preparar(path.join(AQUI, '_bola-variantes/meio-tom.png'))]];
...
for (const [i, [id, png]] of candidatas.entries()) {
  ...
  camadas.push({ input: rotulo(id.toUpperCase(), 300, 15, id.startsWith('meio-tom') ? CREME : LARANJA),
    left: PAD, top: y - 4 });
...
const saida = path.join(AQUI, '_prova-icones.png');

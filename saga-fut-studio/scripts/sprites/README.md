...
node $S/check-sprite.mjs  personagens/<slug>/rigs/andar/w*.png  personagens/<slug>/rigs/correr/r*.png  videos/<videoId>/kf/*.png
```
...
| `gen-react.mjs` | Pose de REAÇÃO reutilizável (biblioteca) | `<slug> <emocao> "<desc>"` → `personagens/<slug>/poses/<emocao>.png` |
| `gen-idle.mjs` | **Folha 2x2 de IDLE** (respiração: ombros + piscada) | `<slug> "<kit>" "<num>" [dir] [nota]` → `personagens/<slug>/rigs/idle/_sheet.png` |
| `gen-walk.mjs` | Folha 2x2 de caminhada (direção travada) | `<slug> "<kit>" "<num>"` → `personagens/<slug>/rigs/andar/_sheet.png` |
| `gen-acao.mjs` | **Folha 2x2 de um GESTO** (4 quadros, 1 render) | `<slug> <nome> "<desc>" "<f1\|f2\|f3\|f4>" [travado]` → `personagens/<slug>/acoes/<nome>/_sheet.png` |
| `gen-run.mjs` | Folha 2x2 de corrida (inclinado) | `<slug> "<kit>" "<num>"` → `personagens/<slug>/rigs/correr/_sheet.png` |
...
beat de repouso vivo. Confira em `personagens/<slug>/rigs/idle/_card.png`: **os ombros mudam entre os quadros?**
...
Igual andar/correr, poses de reação viram biblioteca reutilizável em `personagens/<slug>/poses/<emocao>.png`.
...
Fluxo: `gen-react <slug> <emocao> "..."` → `slice-pose personagens/<slug>/poses/<emocao>.png videos/<id>/kf/<slug>-<emocao>.png`.
...
- **Movimento reutilizável** — `personagens/<slug>/rigs/idle/`, `personagens/<slug>/rigs/andar/` e `personagens/<slug>/rigs/correr/`: gera 1x, reusa em qualquer vídeo.
- **Validar antes de renderizar** — `check-sprite` é gate, FAIL não passa.

## Bibliotecas prontas
- Idle (respiração): `saga-fut/personagens/<slug>/rigs/idle/i1..i4.png`
- Andar: `saga-fut/personagens/<slug>/rigs/andar/w1..w4.png`
- Correr: `saga-fut/personagens/<slug>/rigs/correr/r1..r4.png`
- Gestos: `saga-fut/personagens/<slug>/acoes/<nome>/<nome>1..4.png`
